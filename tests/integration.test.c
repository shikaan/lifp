...
value_t *execute(const char *input) {
  ...
  while (line != NULL) {
    ...
    tryAssert(evaluate(node, global_environment), intermediate_result);
...
int main() {
  ...
  expectEqlUint(nested_list->as.list->data[0].as.list->data->type, VALUE_TYPE_NUMBER, "with correct type");
