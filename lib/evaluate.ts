import {AST, Symbol, SymbolType, Token, TokenTag, TokenType} from "./types.ts";
...
const ENVIRONMENT: Environment = {
  [Symbol.PLUS]: (args: AST) => {
    if (!areNumbers(args) || args.length < 2) throw new InvalidArgumentException();
    return [[TokenTag.NUMBER, args.reduce((sum, [, value]) => sum + value, 0)]];
  },
  [Symbol.MINUS]: (args: AST) => {
    if (!areNumbers(args) || args.length < 2) throw new InvalidArgumentException();
    const [first, ...rest] = args;
    return [[TokenTag.NUMBER, rest.reduce((diff, [, value]) => diff - value, first[1])]];
  },
  [Symbol.WILDCARD]: (args: AST) => {
    if (!areNumbers(args) || args.length < 2) throw new InvalidArgumentException();
    return [[TokenTag.NUMBER, args.reduce((prod, [, value]) => prod * value, 1)]];
  },
  [Symbol.SLASH]: (args: AST) => {
    if (!areNumbers(args) || args.length < 2) throw new InvalidArgumentException();
    const [first, ...rest] = args;
    return [[TokenTag.NUMBER, rest.reduce((prod, [, value]) => prod / value, first[1])]];
  },
  [Symbol.NOT]: (args: AST) => {
...
const isRoutine = (a: unknown): a is Token<SymbolType> => a[0] === TokenTag.SYMBOL

export const evaluate = (ast: AST): AST => {
  let argsTobeReduced = ast, routine: (ast: AST) => AST = null;
  if (isRoutine(ast[0])) {
